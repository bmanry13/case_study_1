...
# Remove Leading/Trailing White Space from Factor/Character values
beers[sapply(beers, class)%in%c("factor", "character")] <- lapply(beers[sapply(beers, class)%in%c("factor", "character")], 
                                                                  function(x) gsub("^\\s|\\s$","",as.character(x)))
breweries[sapply(breweries, class)%in%c("factor", "character")] <- lapply(breweries[sapply(breweries, class)%in%c("factor", "character")], 
                                                                  function(x) gsub("^\\s|\\s$","",as.character(x)))
...
plot.df <- beers %>% group_by(State) %>% summarise(median.abv = median(ABV, na.rm = TRUE)) %>% ungroup()
